...
def cross_time(t_array, x_array, lx):
    """
    find the first time a particle crossed lx in any of the rows of x_array
    :param t_array:
    :param x_array:
    :param lx:
    :return:
    """
    indices = np.argmax(x_array > lx, axis=1)
    first_l_time = t_array[np.arange(t_array.shape[0]), indices]
    t1 = np.min(first_l_time)
    return t1