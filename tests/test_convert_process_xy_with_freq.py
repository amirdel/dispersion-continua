...
def test_convert_xyvf():
    ...
    vy_array = np.array([4.0, 3.0, 2.0, 1.0], dtype=np.float)
    deltaT = 1.0
    dx_array, dy_array, v_array, freq_array = average_dx_dy_array_with_freq_v(dt_array, vx_array, vy_array,
                                                                     x_start, y_start, deltaT)
    dx2, dy2, v2, freq2 = remove_duplicate_xyv(dx_array, dy_array, v_array, freq_array)
    expected_dx = np.array([1.0, 2.5, 3.0, 3.9])
    expected_dy = np.array([4.0, 2.5, 2.0, 1.1])
    expected_freq = np.array([10., 1., 1., 1.])
    expected_v2 = np.sqrt(vx_array**2 + vy_array**2)
    diff_v_norm = np.linalg.norm(v2 - expected_v2)
    diff_dx_norm = np.linalg.norm(dx2 - expected_dx)
    diff_dy_norm = np.linalg.norm(dy2 - expected_dy)
    print "norm(diff_dx): ", diff_dx_norm
    tol = 1e-12
